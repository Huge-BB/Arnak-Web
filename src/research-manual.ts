...
const COST_KEYS = ['coin', 'compass', 'tablet', 'arrowhead', 'jewel', 'usableIdol', 'discardCard'] as const;
...
export function applyResearchManualData(track:ResearchTrackDefinition,manualData:ResearchManualData,options:{requireVerified?:boolean}={}):ResearchTrackDefinition{
 ...
 const requireVerified=options.requireVerified??false; const nodes=new Map(allNodes(next).map(node=>[node.id,node])); const bridges=next.bridges??[]; const topology=new Map(bridges.map(bridge=>[bridgeKey(bridge.from,bridge.to),bridge])); const seenBridges=new Set<string>(); const seenNodes=new Set<string>();
 ...
 for(const manualBridge of overlay.bridges){ const key=bridgeKey(manualBridge.from,manualBridge.to); if(seenBridges.has(key)) throw new Error(`Duplicate manual research bridge: ${key}`); seenBridges.add(key); let target=topology.get(key); if(!target&&manualBridge.to===researchTempleNode(boardId)&&nodes.has(manualBridge.from)){target={id:key,from:manualBridge.from,to:manualBridge.to};bridges.push(target);topology.set(key,target);} if(!target) throw new Error(`Manual research bridge does not exist in ${boardId} topology: ${key}`); validateCost(manualBridge.cost,key); for(const [index,cost] of (manualBridge.alternativeCosts??[]).entries())validateCost(cost,`${key} alternative ${index+1}`); if(requireVerified&&!manualBridge.verified) throw new Error(`Research bridge is not verified: ${key}`); target.cost={...manualBridge.cost}; target.alternativeCosts=manualBridge.alternativeCosts?.map(cost=>({...cost})); if(manualBridge.allowedTokens!==undefined)target.allowedTokens=[...manualBridge.allowedTokens]; target.verified=manualBridge.verified; }
 return next;
}
...
export function assertVerifiedResearchBridge(bridge:ResearchBridgeDefinition){if(!bridge.verified)throw new Error(`Research bridge has not been verified: ${bridge.id}`);validateCost(bridge.cost??{},bridge.id);for(const [index,cost] of (bridge.alternativeCosts??[]).entries())validateCost(cost,`${bridge.id} alternative ${index+1}`);}
