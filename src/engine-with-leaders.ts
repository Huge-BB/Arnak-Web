import { assertNoUnresolvedPendingChoice } from './engine.ts';
...
function contextWithIdolReward(context:EngineContext,idolId:string,rewardCode:string):EngineContext{const idol=context.idols?.[idolId];if(!idol)return context;return{...context,idols:{...context.idols,[idolId]:{...idol,rewardCode}}};}
...
export function reduceWithLeaders(state:GameState,action:LeaderAwareGameAction,context:EngineContext):GameState{
 if('playerId'in action)assertNoUnresolvedPendingChoice(state,action.playerId);
 ...
 if(leader?.id==='falconer'&&leader.data.trackingGuardianChoiceThisTurn===true){if(action.guardianChoiceIndex!==0&&action.guardianChoiceIndex!==1)throw new Error('Tracking requires choosing one of the top two guardians');next.discovery.guardianDeck=chooseTopTwo(next.discovery.guardianDeck,action.guardianChoiceIndex,'Tracking');leader.data.trackingGuardianChoiceThisTurn=false;}
 if(leader?.id==='mystic'&&leader.data.blindsightIdolExileThisTurn===true){leader.data.blindsightIdolExileThisTurn=false;if(action.useBlindsight){const idolId=assignedFaceUpIdol(next,action.siteId)??next.discovery.idolDeck[0];if(!idolId)throw new Error('Blindsight requires a face-up idol from discovery');return discoverWithAssignedIdols(next,action as DiscoverAction,contextWithIdolReward(context,idolId,'e'));}}
