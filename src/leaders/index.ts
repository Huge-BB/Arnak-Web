...
function buildLeaderStartingDeck(state:GameState,playerId:PlayerId,rules:LeaderRules,context:EngineContext,seed:string){if(!rules.startingCardNames)return;const fear=findFearCardsInStartingDeck(state,playerId,context);if(fear.length!==2)throw new Error(`Leader setup expected two Fear cards for ${playerId}`);const leaderCards=resolveLeaderCards(context,rules.startingCardNames,rules.id),player=state.players[playerId];if(rules.id==='baroness'){const special=leaderCards.find(id=>context.cards[id]?.name==='Special Delivery');if(!special)throw new Error('Baroness setup requires Special Delivery');const rest=shuffleWithSeed([...fear,...leaderCards.filter(id=>id!==special)],`${seed}:leader:${playerId}`);player.hand=[special,...rest.slice(0,4)];player.deck=rest.slice(4);}else{const cards=shuffleWithSeed([...fear,...leaderCards],`${seed}:leader:${playerId}`);player.hand=cards.slice(0,5);player.deck=cards.slice(5);}player.playedCards=[];}
