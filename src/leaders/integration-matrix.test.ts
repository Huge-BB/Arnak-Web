...
const cards:EngineContext['cards']={fear1:{id:'fear1',name:'Fear',type:'Fear',expansion:'Base Game'},fear2:{id:'fear2',name:'Fear',type:'Fear',expansion:'Base Game'},draw:{id:'draw',name:'Draw Target',type:'Starter',expansion:'Base Game'},art1:{id:'art1',name:'Archive 1',type:'Artifact',expansion:'Base Game'},art2:{id:'art2',name:'Archive 2',type:'Artifact',expansion:'Base Game'},art3:{id:'art3',name:'Archive 3',type:'Artifact',expansion:'Base Game'}};
...
function leaderState(id:LeaderId){
 const s=createGame(['p1']);s.phase='playing';s.currentPlayer='p1';s.setupSeed='matrix';s.players.p1.hand=['fear1','fear2'];s.players.p1.deck=['draw'];s.market.artifactDeck=['art1','art2','art3'];
 setupLeader(s,'p1',id,context,'matrix');s.players.p1.deck=['draw'];s.players.p1.idols.push({id:'idol',faceUp:true});return s;
}
...
test('Professor blue idol integrates through public engine API',()=>{const s=useIdol('professor','leaderUnique');const suitcase=s.players.p1.leader!.data.suitcase as {compass:number;tablet:number};assert.equal(s.players.p1.resources.coin,1);assert.equal(suitcase.compass,1);assert.equal(s.pendingRewards[0].code,'leader:UPGRADE_RESOURCE');});
test('Explorer blue idol refreshes a used snack and grants a boot through public engine API',()=>{const s=leaderState('explorer');const snacks=s.players.p1.leader!.data.snacks as Array<{id:string;used:boolean;siteId?:string}>;const snack=snacks[0];snack.used=true;snack.siteId='x';const next=applyEngineCommand(s,{type:'action',action:{type:'LEADER_USE_IDOL',playerId:'p1',idolId:'idol',slotIndex:2,effect:'leaderUnique',snackId:snack.id as 'free'|'coin'|'compass'}},context);assert.equal(next.players.p1.resources.compass,1);assert.equal(next.players.p1.resources.coin,1);assert.equal(next.actionWindow?.temporaryTravel.boot,1);const refreshed=(next.players.p1.leader!.data.snacks as Array<{id:string;used:boolean;siteId?:string}>).find(x=>x.id===snack.id)!;assert.equal(refreshed.used,false);assert.equal(refreshed.siteId,undefined);});
