...
export function activateTentSiteForLeader(state:GameState,playerId:PlayerId,siteId:string,context:EngineContext):GameState{const next=structuredClone(state),site=next.sites[siteId];if(!site?.isTentSite||!site.rewardCode)throw new Error('Hike requires a printed camp site');resolveRewardCode(next,playerId,siteId,site.rewardCode,context);return next;}
