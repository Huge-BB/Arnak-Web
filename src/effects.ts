...
export function resolveOwnedCardExile(state:GameState,playerId:PlayerId,cardId:CardId,context:EngineContext){const card=context.cards[cardId],player=state.players[playerId];if(player?.leader?.id==='captain'&&card?.expansion==='Expedition Leaders'&&card.name==='Hidden Fear'){gainFearCards(state,playerId,1,context);gainResource(state,playerId,'compass',1);return;}if(player?.leader?.id==='mystic'&&card?.type==='Fear'){const index=state.market.exiled.lastIndexOf(cardId);if(index>=0)state.market.exiled.splice(index,1);const pile=(player.leader.data.ritualPile??=[]) as CardId[];pile.push(cardId);return;}if(card?.expansion!=='Surprise Shipment')return;if(card.name==='Ominous Chalice'){gainFearCards(state,playerId,1,context);gainResource(state,playerId,'coin',1);}else if(card.name==='Ominous Medallion')gainFearCards(state,playerId,1,context);else if(card.name==='Ominous Warpaint')gainFearCards(state,playerId,2,context);}
