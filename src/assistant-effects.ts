...
import { getCardEffects, applyCardEffects, resolveOwnedCardExile } from './effects.ts';
...
export function resolveAssistantEffect(state:GameState,playerId:PlayerId,assistantId:string,effect:AssistantEffect,sourceKind:'assistant'|'guardian'='assistant'):GameState{
 switch(effect.type){
  ...
  case'SEQUENCE':for(const child of effect.effects)resolveAssistantEffect(state,playerId,assistantId,child,sourceKind);return state;
...
function exileOrDiscard(state:GameState,playerId:PlayerId,cardId:CardId,mode:'exile'|'discard',context:EngineContext){const player=state.players[playerId],zones=mode==='exile'?[player.hand,player.playedCards]:[player.hand];for(const zone of zones){const index=zone.indexOf(cardId);if(index>=0){zone.splice(index,1);if(mode==='exile'){state.market.exiled.push(cardId);resolveOwnedCardExile(state,playerId,cardId,context);}else player.discard.push(cardId);return;}}throw new Error(mode==='exile'?'Assistant exile requires a card in hand or play area':'Assistant discard requires a card in hand');}
...
export function resolvePendingAssistantEffect(state:GameState,playerId:PlayerId,pendingIndex:number,choice:AssistantEffectChoice,context:EngineContext):GameState{
 ...
 if(effect.type==='GAIN_RESOURCES'||effect.type==='GAIN_TRAVEL'||effect.type==='DRAW_CARD'||effect.type==='SEQUENCE')return resolveAssistantEffect(next,playerId,payload.assistantId,effect);
 switch(effect.type){
  case'CHOOSE':if(choice.type!=='assistant-option')throw new Error('Assistant choice requires an option');return resolveAssistantEffect(next,playerId,payload.assistantId,chooseOption(effect,choice.optionIndex));
  case'PAY_RESOURCE_CHOOSE':if(choice.type!=='assistant-option')throw new Error('Assistant choice requires an option');payResources(next,playerId,effect.cost);return resolveAssistantEffect(next,playerId,payload.assistantId,chooseOption(effect,choice.optionIndex));
