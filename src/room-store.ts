...
export type StoredMember = { token: string; playerId?: PlayerId; role: 'player' | 'spectator'; name: string };
export type RoomEvent = { version: number; at: string; kind: 'start' | 'command'; command?: EngineCommand };
export interface StoredRoom { id: string; name: string; seats: number; hostPlayerId: PlayerId; members: StoredMember[]; state?: GameState; version: number; events: RoomEvent[]; snapshots: { version: number; state: GameState }[]; }
