...
if(!Object.keys(data).length)data=structuredClone((embeddedCalibration as {boards:Record<string,Mark[]>}).boards);
...
const research=(id:keyof typeof RESEARCH_LANES)=>{
  ...
  const setupPieces:Mark[] = id==='snake'
    ...
    : id==='monkey'
      ...
      : id==='lizard'
        ? [{id:'lizard-research-track-guardian',kind:'token',asset:'guardian-back',label:'Lizard research guardian',x:52,y:59,width:220,height:220,rotation:0}]
        : [];
  return [...cells,...bonusSlots,...temple,...setupPieces];
};
