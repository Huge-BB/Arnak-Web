...
const publicPath=(path:string)=>`${import.meta.env.BASE_URL.replace(/\/$/,'')}/${path.replace(/^\//,'')}`;
...
function art(cardId:string){const asset=assetMap[`card:${cardId}:face`];if(!asset)return'';if(asset.url)return`background-image:url('${publicPath(asset.url)}')`;const col=asset.cardIndex%asset.sheetWidth,row=Math.floor(asset.cardIndex/asset.sheetWidth);return`background-image:url('${publicPath(asset.sheetUrl)}');background-size:${asset.sheetWidth*100}% ${asset.sheetHeight*100}%;background-position:${asset.sheetWidth===1?0:col/(asset.sheetWidth-1)*100}% ${asset.sheetHeight===1?0:row/(asset.sheetHeight-1)*100}%`;}
...
function labUrl(batch:AuditItem[]){const params=new URLSearchParams({audit:'batch'}),cardIds=batch.flatMap(item=>item.cardId?[item.cardId]:[]),leader=batch.find(item=>item.leader)?.leader;if(cardIds.length)params.set('cards',cardIds.join(','));if(leader)params.set('leader',leader);return`${publicPath('lab.html')}?${params}`}
