...
function mayReplaceLocalSave(){return !localStorage.getItem('arnak.local-game.v2')||localStorage.getItem('arnak.local-game.home')!=='1'||window.confirm('开始新对局会覆盖上次自动存档，确定继续吗？')}
function start(){if(!mayReplaceLocalSave())return;localStorage.removeItem('arnak.local-game.home');const players=Array.from({length:setupPlayerCount},(_,i)=>`p${i+1}`);const leaders=Object.fromEntries(players.flatMap(id=>setupLeaders[id] ? [[id,setupLeaders[id]]] : []));const seed=setupSeed||randomLabSeed(),marketExpansions=['Base Game',...(setupLeadersMarket?['Expedition Leaders']:[]),...(setupSurpriseShipment?['Surprise Shipment']:[])];state=createGame(players);state.sites=createBaseBoardSites(players.length,seed,mainBoard);state=applyEngineCommand(state,{type:'action',action:{type:'START_GAME',seed,researchBoard,moonStaff:setupMoonStaff,leaders,marketExpansions}},context);screen='game';pendingSelection=[];artifactId=undefined;leaderStartingCardId=undefined;message='';render()}
function startSolo(){if(!mayReplaceLocalSave())return;localStorage.removeItem('arnak.local-game.home');const seed=setupSeed||randomLabSeed();researchLab=false;const unsupportedResearchBoard=researchBoard!=='bird'&&researchBoard!=='snake',soloResearchBoard=unsupportedResearchBoard?'bird':researchBoard;if(unsupportedResearchBoard)researchBoard=soloResearchBoard;const marketExpansions=['Base Game',...(setupLeadersMarket?['Expedition Leaders']:[]),...(setupSurpriseShipment?['Surprise Shipment']:[])];state=createSoloGame({seed,difficulty:soloDifficulty,board:mainBoard,researchBoard:soloResearchBoard,leader:setupLeaders.p1||undefined,marketExpansions,context});screen='game';pendingSelection=[];artifactId=undefined;leaderStartingCardId=undefined;message=`单人 solo：难度 ${soloDifficulty}，对手先行动。${unsupportedResearchBoard?' 已切换至基础研究板。':''}`;render()}
function randomLabSeed(){return crypto.randomUUID?.() ?? `${Date.now()}-${Math.random().toString(36).slice(2)}`;}
function startResearchLab(seed=labSeed||randomLabSeed()){labSeed=seed;researchLab=true;setupPlayerCount=1;state=createGame(['p1']);state.sites=createBaseBoardSites(1,seed,mainBoard);const leaders=setupLeaders.p1?{p1:setupLeaders.p1}:undefined,marketExpansions=['Base Game','Expedition Leaders','Surprise Shipment'];state=applyEngineCommand(state,{type:'action',action:{type:'START_GAME',seed,researchBoard,moonStaff:setupMoonStaff,leaders,marketExpansions}},context);const player=state.players.p1;player.resources={coin:40,compass:40,tablet:40,arrowhead:40,jewel:40,fear:0};const availableMarketDeckCards=[...state.market.itemDeck,...state.market.artifactDeck],sample=shuffleWithSeed(availableMarketDeckCards,`${seed}:lab:cards`).slice(0,12),sampleSet=new Set(sample);state.market.itemDeck=state.market.itemDeck.filter(cardId=>!sampleSet.has(cardId));state.market.artifactDeck=state.market.artifactDeck.filter(cardId=>!sampleSet.has(cardId));player.deck=[...player.deck,...sample];player.idols=[...player.idols,...Object.keys(context.idols??{}).slice(0,5).map(id=>({id,inSlot:false}))];const params=new URLSearchParams(location.search),auditCards=(params.get('cards')??params.get('card')??'').split(',').filter(cardId=>context.cards[cardId]);for(const auditCard of auditCards){player.hand=player.hand.filter(id=>id!==auditCard);player.deck=player.deck.filter(id=>id!==auditCard);player.playedCards=player.playedCards.filter(id=>id!==auditCard);state.market.items=state.market.items.filter(id=>id!==auditCard);state.market.artifacts=state.market.artifacts.filter(id=>id!==auditCard);state.market.itemDeck=state.market.itemDeck.filter(id=>id!==auditCard);state.market.artifactDeck=state.market.artifactDeck.filter(id=>id!==auditCard);}if(auditCards.length){const targetHandSize=Math.max(5,auditCards.length),fillerCount=Math.max(0,targetHandSize-auditCards.length),fillers=player.hand.slice(0,fillerCount),excess=player.hand.slice(fillerCount);player.hand=[...auditCards,...fillers];player.deck=[...excess,...player.deck];}screen='game';pendingSelection=[];artifactId=undefined;leaderStartingCardId=undefined;message=auditCards.length?`定向语义审计：已装入 ${auditCards.map(id=>context.cards[id]?.name??id).join('、')}；资源与测试牌组已准备。`:`研究轨实验室：seed ${seed}；全扩展市场；保留${leaders?'领袖':'基础'}起始牌组，并从市场牌堆移入随机测试牌。`;render()}
...
function card(id:string,action:'play'|'buy'){const d=context.cards[id],fear=d?.type==='Fear'||(d?.expansion==='Expedition Leaders'&&d?.name==='Hidden Fear');return`<button class="card ${fear?'fear':''}" ${fear?'disabled':''} data-card-id="${id}" data-card-action="${action}" title="${cardHoverText(id,d?.name??id)}"><i style="${sprite(assets[`card:${id}:face`])}"></i></button>`}
...
function player(id:PlayerId){const p=state.players[id],current=id===state.currentPlayer,leader=p.leader?.id,as=p.assistants.map(a=>`<button class="assistant ${a.exhausted?'exhausted':''} ${a.level}" ${!current||a.exhausted?'disabled':''} data-assistant="${a.id}" title="Activate assistant">♙</button>`).join('');return`<section class="player ${p.color.toLowerCase()} ${current?'current':''} ${leader?`leader-${leader}`:''}">${id===state.firstPlayer?`<div class="player-id first-player-marker" title="起始玩家"><img src="${publicAsset('/assets/starting-player-marker.png')}" alt="起始玩家"></div>`:''}<div class="player-resources"><span title="coin">● ${p.resources.coin}</span><span title="compass">◉ ${p.resources.compass}</span><span title="tablet">▰ ${p.resources.tablet}</span><span title="arrowhead">▲ ${p.resources.arrowhead}</span><span title="jewel">◆ ${p.resources.jewel}</span><span title="Fear">☠ ${p.resources.fear}</span></div><div class="assistants">${as}</div></section>`}
...
pending = () => {
  ...
  if (queued.code === 'leader:OPTIONAL_EXILE_FAR_LEFT_ITEM') {
    ...
    return `<section class="pending-panel connections-refill"><strong>Connections：是否放逐最左侧物品并立即刷新？</strong><p>确认会揭示一张新市场牌，因此本回合将无法撤回。</p><div>${item}${pendingButton('保留当前市场',{type:'skip'})}</div></section>`;
  }
...
// Presentation-quality setup and final scoring live at the final render edge
// so every mode (local, solo and restored games) shares the same UI.
type SetupHelpKey = ResearchBoardId | LeaderId | 'base-explorer' | 'expedition-leaders' | 'surprise-shipment';
let setupHelpKey: SetupHelpKey | undefined;
const setupHelp: Record<SetupHelpKey,{title:string;body:string}> = {
  bird:{title:'鸟庙研究板',body:'经典研究路线。放大镜与笔记本沿分支向上推进，在高层获得更强奖励并争夺失落神庙席位。'},
  snake:{title:'蛇庙研究板',body:'包含救援助手、神像费用与恐惧牌等特殊格。部分助手会在开局移到研究板，并以横置状态等待获得。'},
  monkey:{title:'猴庙研究板',body:'路线具有更多分叉与替代费用，并在开局放置一件费用为 3 的神器；部分格允许按不同交通或资源组合推进。'},
  lizard:{title:'蜥蜴神庙研究板',body:'研究途中会遭遇守卫。放大镜到达对应格后可以按正常守卫规则击败它，否则会阻挡后续推进。'},
  'base-explorer':{title:'基础探险家',body:'使用标准个人版图、两名考古学家和基础起始牌组，没有额外的领袖专属能力。'},
  captain:{title:'Captain · 队长',body:'拥有第三名考古学家与专家行动区，擅长借用助手效果和灵活安排考古学家。'},
  falconer:{title:'Falconer · 鹰女',body:'通过行动与守卫推动猎鹰轨，并在召回猎鹰时选择已经越过的奖励。'},
  baroness:{title:'Baroness · 女爵',body:'善于获取与回收物品，并通过每轮收入和特别快递强化市场运营。'},
  professor:{title:'Professor · 教授',body:'使用手提箱储存资源并经营私人神器档案，研究推进会逐步解锁额外收益。'},
  explorer:{title:'Explorer · 探险家',body:'只有一名考古学家，但可以使用零食移动并再次激活地点，强调路线规划。'},
  mystic:{title:'Mystic · 神秘人',body:'将恐惧牌用于仪式并解锁更强效果；拥有五个神像槽，其中蓝色槽可直接执行仪式。'},
  'expedition-leaders':{title:'Expedition Leaders',body:'加入六位非对称领袖、领袖起始牌与对应市场牌。选择领袖后，其专属版图和起始牌组会在开局设置。'},
  'surprise-shipment':{title:'Surprise Shipment',body:'加入一组新的物品与神器，包含购买折扣、市场操作、额外行动及更多组合效果。'},
};
const setupHelpButton=(key:SetupHelpKey)=>`<button type="button" class="setup-help-button" data-setup-help="${key}" title="查看简介" aria-label="查看简介">?</button>`;
const setupThumb=(src:string,label:string)=>`<i class="setup-choice-thumb" style="background-image:url('${publicAsset(src)}')" role="img" aria-label="${label}"></i>`;
const renderVisualSetup = () => {
  const playerIds=Array.from({length:setupPlayerCount},(_,index)=>`p${index+1}`);
  const hasLocalSave=Boolean(localStorage.getItem('arnak.local-game.v2'));
  const leaderIds=['captain','falconer','baroness','professor','explorer','mystic'] as LeaderId[];
  const leaderNames:Record<LeaderId,string>={captain:'队长',falconer:'鹰女',baroness:'女爵',professor:'教授',explorer:'探险家',mystic:'神秘人'};
  const leaderField=(id:string,index:number)=>{const choices:Array<LeaderId|''>=['',...leaderIds];return`<section class="setup-leader-field"><strong>玩家 ${index+1} · 选择领袖</strong><div class="setup-leader-grid">${choices.map(leader=>{const key=(leader||'base-explorer') as SetupHelpKey,src=leader?`/assets/boards/leader-${leader}.jpg`:'/assets/boards/player-base.jpg',selected=(setupLeaders[id]??'')===leader;return`<div class="setup-leader-option ${selected?'selected':''}"><button type="button" data-setup-leader-choice="${id}" data-leader="${leader}" aria-pressed="${selected}" title="选择${leader?leaderNames[leader]:'基础探险家'}"><i style="background-image:url('${publicAsset(src)}')"></i><b>${leader?leaderNames[leader]:'基础'}</b></button>${setupHelpButton(key)}</div>`;}).join('')}</div></section>`;};
  const help=setupHelpKey?`<section class="setup-help-dialog" role="dialog" aria-modal="true"><div><button data-setup-help-close title="关闭">×</button><h2>${setupHelp[setupHelpKey].title}</h2><p>${setupHelp[setupHelpKey].body}</p></div></section>`:'';
  app.innerHTML=`<main class="setup-screen setup-screen--redesigned"><section class="setup-card setup-card--expedition setup-card--visual"><header class="setup-hero"><div class="setup-kicker">LOST RUINS OF ARNAK</div><h1>准备远征</h1><p>${setupPlayerCount===1?'单人模式：与自动机对手进行五轮远征。':'本地热座：在同一设备上轮流进行远征。'}</p></header><nav class="play-mode-switch"><button class="is-current" disabled><strong>本地设置</strong><small>单人或同设备热座</small></button><button data-room-open><strong>联机房间</strong><small>局域网多人同步</small></button></nav>${hasLocalSave?'<button class="setup-continue" data-local-game-continue><strong>继续上次对局</strong><small>读取浏览器中的自动存档</small></button>':''}<div class="setup-fields setup-fields-visual"><label class="setup-field"><span>玩家人数</span><select data-setup-players>${[1,2,3,4].map(count=>`<option value="${count}" ${setupPlayerCount===count?'selected':''}>${count} 人${count===1?' · Solo':''}</option>`).join('')}</select></label><label class="setup-field"><span>主板图</span><select data-setup-main><option value="bird" ${mainBoard==='bird'?'selected':''}>普通版</option><option value="snake" ${mainBoard==='snake'?'selected':''}>进阶版</option></select></label><label class="setup-field setup-field-visual"><span>研究板 ${setupHelpButton(researchBoard)}</span><span class="setup-choice-line">${setupThumb(`/assets/boards/${researchBoard}-board.jpg`,setupHelp[researchBoard].title)}<select data-setup-research>${(['bird','snake','monkey','lizard'] as ResearchBoardId[]).map(id=>`<option value="${id}" ${researchBoard===id?'selected':''}>${setupHelp[id].title}</option>`).join('')}</select></span></label><label class="setup-field"><span>月杖</span><select data-setup-moon-staff><option value="blue" ${setupMoonStaff==='blue'?'selected':''}>蓝色</option><option value="red" ${setupMoonStaff==='red'?'selected':''}>红色</option></select></label>${playerIds.map(leaderField).join('')}<label class="setup-field setup-seed"><span>随机种子</span><input data-setup-seed value="${setupSeed}" placeholder="留空则每局随机生成" spellcheck="false"></label></div><section class="setup-expansion-title"><strong>扩展内容</strong><span>勾选后将对应市场牌加入本局</span></section><div class="setup-expansions setup-expansion-cards"><label><i style="background-image:url('${publicAsset('/assets/boards/leader-falconer.jpg')}')"></i><span><b><input type="checkbox" data-setup-leaders-market ${setupLeadersMarket?'checked':''}> Expedition Leaders</b><small>领袖扩展市场牌</small></span>${setupHelpButton('expedition-leaders')}</label><label><i style="background-image:url('${publicAsset('/assets/boards/market-combined-bird.jpg')}')"></i><span><b><input type="checkbox" data-setup-surprise-shipment ${setupSurpriseShipment?'checked':''}> Surprise Shipment</b><small>惊喜快递市场牌</small></span>${setupHelpButton('surprise-shipment')}</label></div><div class="setup-mode-actions setup-mode-actions-visual">${setupPlayerCount===1?`<label class="solo-difficulty"><span>自动机难度</span><select data-solo-difficulty>${[0,1,2,3,4,5].map(level=>`<option value="${level}" ${soloDifficulty===level?'selected':''}>${level}</option>`).join('')}</select></label><button class="setup-start" data-solo-start>开始单人对局</button>`:'<button class="setup-start" data-setup-start>开始本地热座</button>'}</div><button class="setup-lab-link" data-research-lab>进入研究轨实验室</button></section>${help}</main>`;
};

function detailedFinalScorePanel() {
  if (state.phase !== 'finished') return '';
  const labels:[keyof ReturnType<typeof scoreFinishedGame>['scores'][string],string][]=[['research','研究'],['templeTiles','神庙奖励'],['idols','神像'],['emptyIdolSlots','空神像槽'],['guardians','守卫'],['cards','卡牌']];
  const breakdown=(score:Record<string,number>)=>`<div class="final-score-breakdown">${labels.map(([key,label])=>`<span><small>${label}</small><b>${score[key]??0}</b></span>`).join('')}</div>`;
  if(state.solo){const result=scoreSoloGame(state,context);return`<section class="final-score final-score-detailed" role="dialog" aria-modal="true"><header><span>${result.humanWon?'远征成功':'远征惜败'}</span><h2>${result.humanWon?'你击败了自动机':'自动机赢得了本局'}</h2></header><div class="final-score-contest"><article class="${result.humanWon?'winner':''}"><h3>你的远征 <strong>${result.human}</strong></h3>${breakdown(result.humanBreakdown as unknown as Record<string,number>)}</article><article class="${result.humanWon?'':'winner'}"><h3>自动机 <strong>${result.rival}</strong></h3>${breakdown(result.rivalBreakdown as unknown as Record<string,number>)}</article></div><button data-local-game-exit>返回首页</button></section>`;}
  const result=scoreFinishedGame(state,context),cards=result.rankedPlayerIds.map((playerId,index)=>{const score=result.scores[playerId],player=state.players[playerId];return`<article class="${result.winnerIds.includes(playerId)?'winner':''}"><h3><em>${index+1}</em><span class="final-player-dot ${player.color.toLowerCase()}"></span>${player.name||`玩家 ${state.playerOrder.indexOf(playerId)+1}`}<strong>${score.total}</strong></h3>${breakdown(score as unknown as Record<string,number>)}</article>`;}).join('');return`<section class="final-score final-score-detailed" role="dialog" aria-modal="true"><header><span>最终结算</span><h2>${result.winnerIds.length>1?'并列胜利':`${state.players[result.winnerIds[0]].name||'玩家'}获胜`}</h2></header><div class="final-score-contest">${cards}</div><button data-local-game-exit>返回首页</button></section>`;
}
function pinSoloRivalToTop(){if(!state.solo||screen!=='game')return;const rivalId=state.solo.rivalPlayerId;app.querySelector(`.player-resource-summary[data-player-id="${rivalId}"]`)?.remove();const panel=app.querySelector<HTMLElement>('.solo-rival-board'),zone=panel?.closest<HTMLElement>('.player-zone'),anchor=app.querySelector('.solo-status')??app.querySelector('header');if(zone&&anchor)anchor.after(zone);}
const renderWithVisualSetupAndScore = render;
render=()=>{renderWithVisualSetupAndScore();if(screen==='game'){app.querySelector('.final-score')?.remove();app.insertAdjacentHTML('beforeend',detailedFinalScorePanel());pinSoloRivalToTop();}};
app.addEventListener('change',event=>{const target=event.target as HTMLSelectElement;if(screen==='setup'&&(target.matches('[data-setup-research]')||target.matches('[data-setup-players]')))render();});
app.addEventListener('click',event=>{const button=(event.target as HTMLElement).closest<HTMLButtonElement>('button');if(!button)return;if(button.dataset.setupHelp){setupHelpKey=button.dataset.setupHelp as SetupHelpKey;render();return}if(button.dataset.setupHelpClose!==undefined){setupHelpKey=undefined;render();return}if(button.dataset.setupLeaderChoice){setupLeaders[button.dataset.setupLeaderChoice]=button.dataset.leader as LeaderId||undefined;render();}});
render();
...
type LeaderCardUiChoice = { choice: string; snackId?: 'free' | 'coin' | 'compass'; label: string; disabled?: boolean; title?: string };
function leaderCardChoices(cardId: string): LeaderCardUiChoice[] {
  ...
  if (leader === 'mystic') {
    const ritualFearCount=((player.leader?.data.ritualPile??[]) as unknown[]).length;
    const ritual:LeaderCardUiChoice={choice:'performRitual',label:'放逐本牌并执行仪式（主行动）',disabled:ritualFearCount<2,title:ritualFearCount<2?'仪式牌堆至少需要 2 张恐惧':'performRitual'};
    if (name === 'Worldly Goods') return [basic('coin', '●'), basic('draw', '▣'), ritual];
    if (name === 'Divine Guidance') return [basic('compass', '◉'), basic('exile', '▣'), ritual];
    if (name === 'Meditation') return [basic('coin', '●'), basic('exile', '▣'), ritual];
    if (name === 'Blindsight') return [basic('compass', '◉'), ritual];
  }
  return [];
}
...
app.addEventListener('click', (event) => {
  ...
  leaderStartingCardId = undefined;
  if(input.choice==='performRitual'){run({type:'LEADER_MYSTIC_EXILE_STARTING_CARD',playerId:state.currentPlayer,cardId:input.cardId} as unknown as GameAction);return;}
...
pending = () => {
  ...
  const archiveButtons = archive.map((id) => `<button class="card ${pendingArchiveSwap.archiveCardId === id ? 'selected-choice' : ''}" data-archive-card="${id}" title="公文包档案：${context.cards[id]?.name ?? id}"><i style="${sprite(assets[`card:${id}:face`])}"></i></button>`).join('');
  const marketButtons = state.market.artifacts.map((id) => `<button class="card ${pendingArchiveSwap.marketCardId === id ? 'selected-choice' : ''}" data-market-artifact="${id}" title="神器市场：${context.cards[id]?.name ?? id}"><i style="${sprite(assets[`card:${id}:face`])}"></i></button>`).join('');
  const ready = pendingArchiveSwap.archiveCardId && pendingArchiveSwap.marketCardId;
  return `<section class="pending-panel archive-swap"><strong>可选择交换一张档案神器与一张市场神器</strong><label>公文包档案</label><div>${archiveButtons}</div><label>神器市场</label><div>${marketButtons}</div><div><button class="pending-confirm" ${ready ? '' : 'disabled'} data-archive-confirm>确认交换</button>${pendingButton('跳过', { type: 'skip' })}</div></section>`;
};
app.addEventListener('click', (event) => {
  const button = (event.target as HTMLElement).closest<HTMLButtonElement>('button');
  if (!button || button.disabled) return;
...
type PaymentDraft = { action: 'research' | 'pending-research' | 'site' | 'discover' | 'guardian' | 'lizard-guardian'; destinationId: string; cost: Record<string, unknown>; cardIds: string[]; cardIndexes: number[]; temporaryTravel?:Partial<Record<'boot'|'car'|'boat'|'plane',number>>; hiredPlanes?:number; discardCardId?: string; discardCardIndex?:number; guardianChoiceIndex?:0|1; siteChoiceIndex?:0|1; useScouting?:boolean; useBlindsight?:boolean; leaderChoiceResolved?:boolean; revealConfirmed?:boolean; researchToken?: 'magnifying'|'journal'; bonusTileId?:string; costAlternativeIndex?:number; feedbackOrigin?: { x:number; y:number } };
...
let pendingRevealLeaderChoice: { draft:PaymentDraft; kind:'tracking'|'scouting' } | undefined;
...
render = () => {
  ...
  if (leaderDiscoveryChoice) {
    ...
    const choices = kind==='tracking'
      ...
      : kind==='scouting'
        ? (site?.level===2?state.discovery.level2Deck:state.discovery.level1Deck).slice(0,2).map((id,index)=>{const asset=assets[`site:${id}:face`],ratio=asset?.width&&asset?.height?`${asset.width}/${asset.height}`:'3/4';return `<button class="site-choice" data-leader-discovery-choice="${index}" title="${context.sites?.[id]?.name??id}" style="--site-choice-ratio:${ratio}"><i style="${sprite(asset)}"></i></button>`;}).join('')
  ...
  if (button.dataset.paymentConfirm !== undefined) {
    ...
    try {
      if(researchPayment.action==='discover'&&!researchPayment.leaderChoiceResolved){
        ...
        if(kind){const draft=researchPayment;researchPayment=undefined;if(kind!=='blindsight'&&!draft.revealConfirmed)pendingRevealLeaderChoice={draft,kind};else leaderDiscoveryChoice={draft,kind};render();return;}
      }
      ...
      researchPayment = undefined;
      if(draft.revealConfirmed){const before=state;eventBeforeState=before;runBeforeUndoPolicy(action);eventBeforeState=undefined;if(!roomSession&&state!==before&&!message){turnStartCode=encodeStateCode(state);turnStartOwner=state.currentPlayer;localTurnUndoLocked=false;}}
      else run(action);
      return;
...
function supplyBoard(){
  ...
  const startCenter={x:(magnifyingStart.x+journalStart.x)/2+330,y:(magnifyingStart.y+journalStart.y)/2,width:16,height:16};
...
render = () => {
  ...
  if (roomSession && screen === 'game' && roomSnapshot?.viewer.role === 'player') {
    ...
    app.querySelector('.header-actions')?.insertAdjacentHTML('beforeend', `<button data-room-auto-pass="${enabled}" title="${enabled ? '取消预约跳过' : '下次轮到你时自动跳过'}">${enabled ? '取消自动跳过' : '自动跳过'}</button>`);
  }
};
...
function leaderComponents(id: PlayerId) {
  ...
  if (leader.id === 'mystic') {
    const pile=(leader.data.ritualPile??[]) as string[];
    return `<span class="mystic-ritual-pile" title="仪式牌堆：${pile.length} 张恐惧">${pile.map((cardId,index)=>`<i style="${sprite(assets[`card:${cardId}:face`])};--ritual-index:${Math.min(index,5)}"></i>`).join('')}${pile.length?`<b>${pile.length}</b>`:''}</span>`;
  }
...
player = (id) => {
  ...
  return `${playerWithPlayedCards(id)}<aside class="player-played-zone" aria-label="${id} 已打出"><strong>已打出</strong><div class="player-played-cards">${played || '<span>—</span>'}</div></aside>`;
};
...
function playerComponentTray(id: PlayerId) {
  ...
  const archive = playerState.leader?.id === 'professor'
    ? ((playerState.leader.data.archive ?? []) as string[]).map((cardId) => `<button class="card professor-archive-card" data-professor-preview="${cardId}" title="教授档案：${context.cards[cardId]?.name ?? cardId}"><i style="${sprite(assets[`card:${cardId}:face`])}"></i></button>`).join('')
    : '';
  const suitcase = (playerState.leader?.data.suitcase ?? {}) as { compass?: number; tablet?: number };
  const archiveGroup = playerState.leader?.id === 'professor'
    ? `<section class="player-component-group professor-archive" aria-label="教授公文包"><header class="professor-suitcase-heading"><img src="${publicAsset('/assets/leader-professor-suitcase-alpha.png')}" alt="教授公文包"><span>${resourceArtwork('compass', suitcase.compass ?? 0)}${resourceArtwork('tablet', suitcase.tablet ?? 0)}</span></header><div>${archive || '<small>—</small>'}</div></section>`
    : '';
  return `<aside class="player-component-tray" aria-label="${id} 持有组件">${archiveGroup}<section class="player-component-group player-guardians"><strong>守卫</strong><div>${guardians || '<small>—</small>'}</div></section><section class="player-component-group player-temporary-travel"><strong>交通工具</strong><div>${temporaryIcons || '<small>—</small>'}</div></section></aside>`;
}
...
// hover preview; touch users tap a card and dismiss the pinned preview.
let professorPreviewCardId: string | undefined;
let professorPreviewPinned = false;
function professorCardPreview() {
  if (!professorPreviewCardId) return '';
  const definition = context.cards[professorPreviewCardId];
  const player = state.players[state.currentPlayer], suitcase = (player.leader?.data.suitcase ?? {}) as { compass?:number };
  const isArchiveCard = player.leader?.id === 'professor' && ((player.leader.data.archive ?? []) as string[]).includes(professorPreviewCardId);
  const cost = definition?.cost ?? 0, maxSuitcase = Math.min(cost, suitcase.compass ?? 0);
  const purchaseOptions = isArchiveCard && !state.pendingRewards.length && (!player.mainActionUsed || (player.extraMainActions ?? 0) > 0)
    ? Array.from({length:maxSuitcase+1},(_,useSuitcase)=>({useSuitcase,general:cost-useSuitcase})).filter(option=>option.general<=player.resources.compass).map(option=>`<button class="professor-buy-option" data-professor-buy="${professorPreviewCardId}" data-suitcase-compass="${option.useSuitcase}">购买：${option.useSuitcase ? `公文包 ${option.useSuitcase} + ` : ''}资源池 ${option.general} 罗盘</button>`).join('')
    : '';
  return `<aside class="card-detail-preview" role="dialog" aria-label="神器详情"><button data-professor-preview-close aria-label="关闭">×</button><i style="${sprite(assets[`card:${professorPreviewCardId}:face`])}"></i><section><strong>${definition?.name ?? professorPreviewCardId}</strong><p>${cardHoverText(professorPreviewCardId, definition?.name ?? professorPreviewCardId)}</p>${purchaseOptions ? `<div class="professor-buy-options">${purchaseOptions}</div>` : ''}</section></aside>`;
}
const renderWithProfessorPreview = render;
render = () => { renderWithProfessorPreview(); if (screen === 'game' && professorPreviewCardId) app.insertAdjacentHTML('beforeend', professorCardPreview()); };
app.addEventListener('pointerover', (event) => {
  if (professorPreviewPinned || (event as PointerEvent).pointerType === 'touch') return;
  const card = (event.target as HTMLElement).closest<HTMLElement>('[data-professor-preview]');
  if (!card || card.dataset.professorPreview === professorPreviewCardId) return;
  professorPreviewCardId = card.dataset.professorPreview; render();
});
app.addEventListener('pointerout', (event) => {
  if (professorPreviewPinned || (event as PointerEvent).pointerType === 'touch') return;
  const card = (event.target as HTMLElement).closest<HTMLElement>('[data-professor-preview]');
  if (!card || card.contains((event as PointerEvent).relatedTarget as Node | null)) return;
  professorPreviewCardId = undefined; render();
});
app.addEventListener('click', (event) => {
  const purchase = (event.target as HTMLElement).closest<HTMLButtonElement>('[data-professor-buy]');
  if (purchase) {
    const cardId = purchase.dataset.professorBuy!, suitcaseCompass = Number(purchase.dataset.suitcaseCompass ?? 0);
    professorPreviewCardId = undefined; professorPreviewPinned = false;
    run({type:'LEADER_PROFESSOR_BUY_ARCHIVE',playerId:state.currentPlayer,cardId,suitcaseCompass});
    return;
  }
  const target = (event.target as HTMLElement).closest<HTMLElement>('[data-professor-preview],[data-professor-preview-close]');
  if (!target) return;
  if (target.dataset.professorPreviewClose !== undefined) { professorPreviewCardId = undefined; professorPreviewPinned = false; render(); return; }
  professorPreviewCardId = target.dataset.professorPreview; professorPreviewPinned = true; render();
});

const renderWithSpecialDeliveryPrompt = render;
render = () => {
  renderWithSpecialDeliveryPrompt();
  if (!specialDeliveryPurchaseCardId || screen !== 'game') return;
  const name=context.cards[specialDeliveryPurchaseCardId]?.name ?? specialDeliveryPurchaseCardId;
  app.insertAdjacentHTML('beforeend', `<section class="pending-panel special-delivery-prompt" role="dialog"><strong>购买 ${name}</strong><p>是否打出 Special Delivery，将购买的物品直接加入手牌？</p><div><button data-special-delivery="yes">使用并购买</button><button data-special-delivery="no">直接购买到牌库</button><button data-special-delivery="cancel">取消</button></div></section>`);
};
app.addEventListener('click',(event)=>{
  const button=(event.target as HTMLElement).closest<HTMLButtonElement>('[data-special-delivery]');
  if(!button||!specialDeliveryPurchaseCardId)return;
  const cardId=specialDeliveryPurchaseCardId,choice=button.dataset.specialDelivery;specialDeliveryPurchaseCardId=undefined;
  if(choice==='cancel'){render();return;}
  run({type:'BUY_CARD',playerId:state.currentPlayer,cardId,useSpecialDelivery:choice==='yes'});
});
...
app.addEventListener('change', event => { const target=event.target as HTMLSelectElement; if(target.matches('[data-research-lab-leader]')) setupLeaders.p1=target.value as LeaderId|''; if(target.matches('[data-research-lab-main-board]')) mainBoard=target.value as 'bird'|'snake'; if(target.matches('[data-research-lab-board]')) researchBoard=target.value as ResearchBoardId; });
...
function applyTurnFocusedPlayerLayout() {
  ...
  if (!main || !players || !hand || !surface) return;
  ...
  if (activeZone) {
    ...
    activeZone.after(surface);
    surface.after(players);
  } else hand.after(surface);
...
render=()=>{renderBeforeUndoPolicy();if(screen!=='game')return;app.querySelector('.activity-log')?.remove();app.querySelector('header')?.insertAdjacentHTML('afterend',detailedActivityLog());app.querySelectorAll('.debug-timeline details,.debug-current-code,.debug-current-actions,.debug-timeline>p').forEach(node=>node.remove());const hiddenLeaderChoice=Boolean(leaderDiscoveryChoice?.draft.revealConfirmed&&(leaderDiscoveryChoice.kind==='tracking'||leaderDiscoveryChoice.kind==='scouting'));const canUndo=!hiddenLeaderChoice&&(roomSession?Boolean(roomSnapshot?.viewer.canUndoTurn):Boolean(turnStartCode&&!localTurnUndoLocked&&turnStartOwner===state.currentPlayer));app.querySelector('.header-actions')?.insertAdjacentHTML('afterbegin',`<button data-undo-turn ${canUndo?'':'disabled'} title="撤回到最近一次信息检查点">撤回本回合</button>`);if(pendingRevealAction||pendingRevealChoice||pendingRevealLeaderChoice)app.insertAdjacentHTML('beforeend',`<section class="reveal-confirm" role="dialog" aria-modal="true"><div><h2>将揭示新信息</h2><p>此操作会抽牌、翻开未知组件或查看牌库顶。确认后，撤回检查点将更新到操作完成后。</p><div><button data-reveal-cancel>取消</button><button class="confirm" data-reveal-confirm>确认执行</button></div></div></section>`);};
app.addEventListener('click',event=>{const button=(event.target as HTMLElement).closest<HTMLButtonElement>('button');if(!button)return;if(button.dataset.undoTurn!==undefined){if(roomSession)void undoLanTurn();else if(turnStartCode&&!localTurnUndoLocked){state=decodeStateCode(turnStartCode);researchBoard=state.research.board;message='已撤回到最近一次信息检查点。';recordReducerEvent({type:'UNDO_TURN'},'checkpoint');render();}return;}if(button.dataset.revealCancel!==undefined){pendingRevealAction=undefined;pendingRevealChoice=undefined;pendingRevealLeaderChoice=undefined;render();return;}if(button.dataset.revealConfirm!==undefined){const action=pendingRevealAction,choice=pendingRevealChoice,leaderChoice=pendingRevealLeaderChoice;pendingRevealAction=undefined;pendingRevealChoice=undefined;pendingRevealLeaderChoice=undefined;if(leaderChoice){leaderChoice.draft.revealConfirmed=true;leaderDiscoveryChoice=leaderChoice;render();return;}if(action){const before=state;eventBeforeState=before;runBeforeUndoPolicy(action);eventBeforeState=undefined;if(!roomSession&&state!==before&&!message){turnStartCode=encodeStateCode(state);turnStartOwner=state.currentPlayer;localTurnUndoLocked=false;}}else if(choice){const before=state;eventBeforeState=before;chooseBeforeUndoPolicy(choice);eventBeforeState=undefined;if(!roomSession&&state!==before&&!message){turnStartCode=encodeStateCode(state);turnStartOwner=state.currentPlayer;localTurnUndoLocked=false;}}return;}});
render();
...
const LOCAL_GAME_HOME_KEY='arnak.local-game.home';
...
const savedLocalGame=roomSession||localStorage.getItem(LOCAL_GAME_HOME_KEY)==='1'?undefined:readLocalGameSave();
...
render=()=>{renderWithSoloBoardAndLocalSave();applySoloRivalBoard();if(screen==='game'&&!roomSession){app.querySelector('.header-actions')?.insertAdjacentHTML('beforeend','<button data-local-game-exit title="返回首页并保留自动存档">返回首页</button>');writeLocalGameSave();}};
app.addEventListener('click',event=>{const button=(event.target as HTMLElement).closest<HTMLButtonElement>('button');if(!button)return;if(button.dataset.localGameExit!==undefined){localStorage.setItem(LOCAL_GAME_HOME_KEY,'1');screen='setup';researchLab=false;message='';render();return;}if(button.dataset.localGameContinue!==undefined){const saved=readLocalGameSave();if(!saved)return;state=saved.state;mainBoard=saved.mainBoard;researchBoard=saved.researchBoard;researchLab=saved.researchLab;turnStartCode=saved.turnStartCode;turnStartOwner=saved.turnStartOwner;localTurnUndoLocked=saved.turnUndoLocked;debugTimeline=saved.debugTimeline??[];localStorage.removeItem(LOCAL_GAME_HOME_KEY);screen='game';message='已读取上次本地对局。';render();}});
render();
...
function enhanceChoicePanels() {
  ...
  panels.forEach((panel) => {
    ...
    const hasRevealedLeaderChoices=panel.classList.contains('leader-discovery-picker')&&Boolean(leaderDiscoveryChoice?.draft.revealConfirmed)&&(leaderDiscoveryChoice?.kind==='tracking'||leaderDiscoveryChoice?.kind==='scouting');
    if(!hasRevealedLeaderChoices&&!panel.querySelector('[data-choice-overlay-cancel]'))panel.insertAdjacentHTML('afterbegin','<button class="choice-overlay-cancel" data-choice-overlay-cancel title="返回上一层选择">取消</button>');
...
app.addEventListener('click', (event) => {
  ...
  if(button.dataset.choiceOverlayCancel!==undefined){
    if(leaderDiscoveryChoice?.draft.revealConfirmed&&(leaderDiscoveryChoice.kind==='tracking'||leaderDiscoveryChoice.kind==='scouting'))return;
    ...
    else {artifactId=undefined;leaderStartingCardId=undefined;specialDeliveryPurchaseCardId=undefined;researchPayment=undefined;leaderDiscoveryChoice=undefined;pendingRevealLeaderChoice=undefined;researchCostChoice=undefined;researchBonusChoice=undefined;leaderIdolDraft=undefined;leaderIdolSnackDraft=undefined;captainSpecialistDraft=undefined;mysticRitualDraft=undefined;baseIdolDraft=undefined;pendingRevealAction=undefined;pendingRevealChoice=undefined;pendingSelection=[];}
    render();return;
  }
...
// Apply the visual entry screen and Solo layout after every legacy enhancer.
renderSetup = renderVisualSetup;
const renderWithFinalPresentation = render;
render = () => {
  renderWithFinalPresentation();
  if (screen !== 'game') return;
  app.querySelectorAll('.final-score').forEach((panel) => panel.remove());
  app.insertAdjacentHTML('beforeend', detailedFinalScorePanel());
  pinSoloRivalToTop();
};
render();

// A directed Lab URL wins over local autosave restoration. This must remain
// after every render/state enhancer so an unrelated saved game cannot replace
// the requested leader audit.
if (location.pathname.endsWith('/lab.html')) {
  const directedLeader = new URLSearchParams(location.search).get('leader') as LeaderId | null;
  if (directedLeader && ['captain','falconer','baroness','professor','explorer','mystic'].includes(directedLeader)) setupLeaders.p1 = directedLeader;
  startResearchLab();
}
