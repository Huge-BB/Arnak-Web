...
const renderVisualSetup = () => {
  ...
  const leaderSelect=(id:string)=>`<option value="">基础探险家</option>${leaderIds.map(leader=>`<option value="${leader}" ${setupLeaders[id]===leader?'selected':''}>${leader}</option>`).join('')}`;
  const leaderField=(id:string,index:number)=>{const leader=setupLeaders[id],key=(leader||'base-explorer') as SetupHelpKey,src=leader?`/assets/boards/leader-${leader}.jpg`:'/assets/boards/player-base.jpg';return`<label class="setup-field setup-field-visual"><span>玩家 ${index+1} · 领袖 ${setupHelpButton(key)}</span><span class="setup-choice-line">${setupThumb(src,leader||'基础探险家')}<select data-setup-leader="${id}">${leaderSelect(id)}</select></span></label>`;};
...
app.addEventListener('change',event=>{const target=event.target as HTMLSelectElement;if(screen==='setup'&&(target.matches('[data-setup-leader]')||target.matches('[data-setup-research]')||target.matches('[data-setup-players]')))render();});
app.addEventListener('click',event=>{const button=(event.target as HTMLElement).closest<HTMLButtonElement>('button');if(!button)return;if(button.dataset.setupHelp){setupHelpKey=button.dataset.setupHelp as SetupHelpKey;render();return}if(button.dataset.setupHelpClose!==undefined){setupHelpKey=undefined;render();}});
render();
