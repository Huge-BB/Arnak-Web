...
function render(){document.documentElement.dataset.theme='jungle';const active=state.players[state.currentPlayer],boardSelectors=researchLab?'':`<select data-main-board><option value="bird" ${mainBoard==='bird'?'selected':''}>普通</option><option value="snake" ${mainBoard==='snake'?'selected':''}>进阶</option></select><select data-board>${(['bird','snake','monkey','lizard'] as ResearchBoardId[]).map(x=>`<option value="${x}" ${researchBoard===x?'selected':''}>${x}</option>`).join('')}</select>`;app.innerHTML=`<main><header><div class="round">${state.round}</div><div class="turn-dot ${active.color.toLowerCase()}"></div><div class="header-actions">${boardSelectors}<button class="${researchToken==='magnifying'?'active':''}" data-token="magnifying">放大镜</button><button class="${researchToken==='journal'?'active':''}" data-token="journal">笔记本</button><button data-action="end">结束回合</button><button data-action="pass">跳过</button><button data-action="reset">重新开始</button></div></header><section class="play-surface">${board()}<aside class="market"><div class="market-row">${state.market.items.map(x=>card(x,'buy')).join('')}</div><div class="market-row">${state.market.artifacts.map(x=>card(x,'buy')).join('')}</div></aside></section><section class="players">${state.playerOrder.map(player).join('')}</section><section class="hand">${active.hand.map(x=>card(x,'play')).join('')}</section>${artifact()}${pending()}${message?`<div class="message">${message}</div>`:''}</main>`}
