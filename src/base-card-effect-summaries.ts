...
function effect(value: CardEffect): string {
  switch (value.type) {
    ...
    case 'RESEARCH_ANY_DISCOUNT_THEN': return `推进任一研究标记时少付 ${bundle(value.discount)}`;
    ...
    case 'ACTIVATE_TENT_SITES': return `激活 ${value.count} 个不同的${value.requireEmpty ? '空' : ''}营地`;
    case 'ACTIVATE_DISCOVERED_LEVEL1_SITE': return '激活一个已发现的一级地点';
    case 'ACTIVATE_OWN_OCCUPIED_SITE': return '激活一个有自己考古学家的地点';
    case 'ACTIVATE_TOP_SITE_DECK': return `激活${value.level===2?'二':'一'}级地点牌堆顶并放回底部`;
    ...
    case 'BUY_ITEM': return `购买${value.includeTop?'市场或物品牌堆顶的':'市场'}物品，少付 ${value.discount} 金币`;
    case 'BUY_ARTIFACT': return `购买${value.includeTop?'市场或神器牌堆顶的':'市场'}神器，少付 ${value.discount} 罗盘`;
    ...
    case 'EXCHANGE_ASSISTANT_WITH_AVAILABLE': return '将自己的一个助手与供给区同级助手交换，并重置双方';
